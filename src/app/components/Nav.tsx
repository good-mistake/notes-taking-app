...
import { setView, clearSelection } from "../redux/viewSlice";
import { useWindowSize } from "./useWindowSize";

const Nav = () => {
  const dispatch = useAppDispatch();
  const { notes, filter } = useAppSelector(selectNotesState);
        ...
        <AnimatePresence>
          ...
          >
            ...
            <motion.li
              ...
              className={filter === "archived" ? "active archived" : "archived"}
              initial={{ opacity: 0, y: -5 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 5 }}
              transition={{ duration: 0.2 }}
          ...
          <ul className="tagLists">
            {tags.map((tag) => (
              <motion.li
                key={`tag-${tag || "untitled"}`}
                onClick={() => handleFilterClick({ tag })}
                className={
                  typeof filter === "object" && filter.tag === tag
                    ? "active"
                    : ""
                }
                initial={{ opacity: 0, y: -5 }}
                ...
                transition={{ duration: 0.2 }}
              >
                <img src={"/assets/images/icon-tag.svg"} />
                <p>{tag}</p>
              </motion.li>
            ))}
          </ul>
